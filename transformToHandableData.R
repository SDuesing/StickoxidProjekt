...
library(reshape2)
library("gplots")
f.list <- list.files(path = "X:/p3_working/SDuesing-home/Data/LfULG/StickoxidProjekt/Data/Stationen",pattern=".csv")

table.final <- NULL
for (i in 1:length(f.list)){
  table <- read.csv(paste("./Data/Stationen/",f.list[i],sep=""),quote="'",sep=";",header = TRUE,dec = ".",na.strings = c("-999.0","  -999.00"),fill = TRUE,stringsAsFactors = T,strip.white = TRUE)
  if(names(table)[length(names(table))]=="Nachweisgrenze"){
    
  }else{
    table$Nachweisgrenze <-NA
  }
...
table.final.final$value[table.final.final$value<table.final.final$Nachweisgrenze]<-NA
final <- dcast(table.final.final,Station + date ~ variable, value.var = "value")
final$Windrichtung[final$Windrichtung==555]<-NA
write.table(final,"StationOverview_20190411.dat",col.names = T,row.names = F,append=F,quote=F,sep="\t")
